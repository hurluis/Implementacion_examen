...
class Hospital:
    ...
    def insert(self, paciente):
        ...
        if self.value is None:
            self.value = paciente
            self.length = 1
            print(" Nodo Raíz(Nivel 0)")  
        else:
            if self.leftchild is None:
                new_node = Hospital(paciente)
                new_node.parent = self
                self.leftchild = new_node
                self.length += 1
                self.queue.enqueue(new_node)
                print(f"El Nivel: {self.obtenerNivel(self.leftchild)}")  
                ...
                self.length += 1
                self.queue.enqueue(new_node)
                print(f"El Nivel: {self.obtenerNivel(self.rightchild)}")  
            else:
                if self.leftchild.length <= self.rightchild.length:
                    nivel = self.obtenerNivel(self.leftchild) + 1  
                    self.leftchild.insert(paciente)
                    self.length += 1
                    print(f"El Nivel: {nivel}")
                else:
                    nivel = self.obtenerNivel(self.rightchild) + 1  
                    self.rightchild.insert(paciente)
                    self.length += 1
                    print(f"El Nivel: {nivel}")
